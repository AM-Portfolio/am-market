...
public class StockFactSheetDividendProcessor implements DataProcessor<StockFactSheetDividend, Void> {
    ...
    public Void process(StockFactSheetDividend data) {
        if (data == null || data.getFactSheetDividend() == null || data.getFactSheetDividend().isEmpty()) {
        ...
        log.info("Processing {} stock cash flow", data.getFactSheetDividend().size());

        try {
            // Save to the database
            stockFinancialPerformanceService.saveFactSheetDividend(data);

            log.info("Processing stock cash flow data. Symbol: {}, Count: {}", 
                data.getSymbol(),
                data.getFactSheetDividend().size());
            // Publish to Kafka
            try {
                stockPortfolioProducer.ifPresent(producer -> producer.sendFactSheetFinancialsUpdate(data.getSymbol(), data));
