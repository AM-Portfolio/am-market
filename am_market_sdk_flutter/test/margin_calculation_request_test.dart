//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:am_market_sdk_flutter/api.dart';
import 'package:test/test.dart';

// tests for MarginCalculationRequest
void main() {
  // final instance = MarginCalculationRequest();

  group('test MarginCalculationRequest', () {
    // List<Position> positions (default value: const [])
    test('to test the property `positions`', () async {
      // TODO
    });

    // String orderType
    test('to test the property `orderType`', () async {
      // TODO
    });

    // String exchange
    test('to test the property `exchange`', () async {
      // TODO
    });


  });

}
