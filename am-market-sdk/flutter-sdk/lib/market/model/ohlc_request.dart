...
class OHLCRequest {
  /// Returns a new [OHLCRequest] instance.
  OHLCRequest({
    this.symbols,
    this.isIndexSymbol,
    this.timeFrame,
  ...
  /// Returns a new [OHLCRequest] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static OHLCRequest? fromJson(dynamic value) {
    if (value is Map) {
  ...
  static List<OHLCRequest> listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = OHLCRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
